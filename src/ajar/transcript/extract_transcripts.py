...
# def extract_transcript_events(auditor_agent_span: EventTreeSpan, transcript_events: list) -> None:
...
STORE_PREFIX_MAPPING = {
    ...
    "target_behavior": r"/AuditStore:(instance:)?target_behavior",
    "message_count": r"/AuditStore:(instance:)?message_count",
    "tool_call_count": r"/AuditStore:(instance:)?tool_call_count",
    "total_rollback_count": r"/AuditStore:(instance:)?total_rollback_count",
    "rollback_retry_counts": r"/AuditStore:(instance:)?rollback_retry_counts",
    "max_messages": r"/AuditStore:(instance:)?max_messages",
    "max_tool_calls": r"/AuditStore:(instance:)?max_tool_calls",
    "max_rollback_retries": r"/AuditStore:(instance:)?max_rollback_retries",
    "max_total_rollback_retries": r"/AuditStore:(instance:)?max_total_rollback_retries",
    "termination_reason": r"/AuditStore:(instance:)?termination_reason",
}
...
def extract_transcript_events(auditor_agent_span: EventTreeSpan, transcript_events: list[Event]) -> None:
    """Traverse the auditor span tree and append transcript-related events.
        ...
        if isinstance(child, EventTreeSpan):
            ...
            if span_type == "transcript":
                ...
                for view, changes in grouped.items():
                    ...
                    elif view == "target_behavior":
                        # target_behavior is metadata, no transcript event needed
                        pass
                    elif view == "message_count":
                        # message_count is internal counter, no transcript event needed
                        pass
                    # New elif branch
                    elif view == "rollback_retry_counts":
                        # Internal counter for rollbacks, no transcript event needed
                        pass
                    # -------------------------
                    else:
...
# def _find_auditor_span(tree: EventTreeSpan) -> EventTreeSpan:
#     solvers = None
#     for root in tree:
...
def sample_to_transcript(sample: TaskState) -> Transcript:
    ...
    # If store has no metadata, init new one instead of keeping None
    if metadata is None:
        # TranscriptMetadata requires transcript_id and created_at
        now = datetime.now()
