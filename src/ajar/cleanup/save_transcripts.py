...
from ajar.transcript.extract_transcripts import sample_to_transcript
...
async def save_transcripts(state: TaskState):
    ...
    try:
        ...
        with open(os.path.join(save_dir, f"transcript_{current_time}_{state.id}.json"), "w", encoding="utf-8") as f:
